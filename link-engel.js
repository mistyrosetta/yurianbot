...
exports.run = async (client, message) => {
    ...
    if(secenekler.length < 1) return message.channel.send("Link Engelleme Açmak İçin `a!link-engelle aç` kapatmak için `a!link-engelle kapat`").then(m => m.delete(10000));
  ...
  if(!linkEngelle[message.guild.id]){
        linkEngelle[message.guild.id] = {
          ...
          fs.writeFile("././ayarlar/linkEngelle.json", JSON.stringify(linkEngelle), (x) => {
  ...
  if(!linkEngelle[message.guild.id]){
    ...
    exports.conf = {
        enabled: true,
        guildOnly: false,
        aliases: ['link-engelle', 'reklamkoruması', 'reklam-koruması', 'linkengel','link'],
        permLevel: 3
      };
