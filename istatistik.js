...
exports.run = async (bot, message, args) => {
  ...
  var osType = await os.type();
    ...
    else if (osBit === 'x86','x32') osBit = '32 Bit'
    else osBit = os.arch();
    ...
    cpuStat.usagePercent(function(err, percent, seconds) {
      ...
      setTimeout(() => {
        ...
        **Kanal Sayısı:** ${bot.channels.size.toLocaleString()}
        `, true)
        .addField('Uygulama sürümleri', stripIndents`
        **Bot Sürümü** 1.0.0
        **Discord.JS sürümü** v${Discord.version}
        **NodeJS sürümü** ${process.version}
        `, true)
        .addField('Kullanılan RAM bellek', `${Math.round(process.memoryUsage().heapUsed / 1024 / 1024).toLocaleString()} MB`, true)
