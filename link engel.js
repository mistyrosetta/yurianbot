...
exports.run = async (bot, message, args) => {
  ...
  if (args[0] == 'aç') {
    var i = await db.set(`reklam_${message.guild.id}`, 'acik')
      ...
      message.channel.send('Reklam Engeli başarıyla açıldı! `Üyeleri Yasakla` yetkisine sahip olanların reklamı engellenmeyecektir.')
    }
  
  if (args[0] == 'kapat') {
    var i = await db.set (`reklam_${message.guild.id}`, 'kapali')
      message.channel.send('Reklam Engeli başarıyla kapatıldı! Artık herkes reklam yapabilir.')
    }
