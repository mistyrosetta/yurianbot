...
let okul = new Date(`${body.tarihiftar}`)
let zaman = ms(okul - Date.now())


 message.channel.send(new Discord.RichEmbed().setImage(`${body.resim}`).setAuthor(`${body.iftar}`).setDescription(`${body.konumi} **${zaman.days}** gün **${zaman.hours}** saat **${zaman.minutes}** dakika **${zaman.seconds}** saniye kaldı!`).setFooter(`API Sahibi: ${body.yapımcı}.`, message.author.avatarURL).setColor('RANDOM'))  
